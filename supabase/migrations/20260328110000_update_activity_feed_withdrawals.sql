-- Migration: Update user_activity_feed to support withdrawal statuses and IDs
-- This allows the UI to show 'pending' withdrawals and provide a 'Cancel' button.

DROP VIEW IF EXISTS "public"."user_activity_feed";

CREATE VIEW "public"."user_activity_feed" AS
 SELECT 
    COALESCE(wl.metadata ->> 'request_id', (wl.id)::text) AS id, -- Use request_id as primary ID for withdrawals
    wl.user_id,
    ...
    COALESCE(wl.metadata ->> 'status'::text, 'completed'::text) AS status, -- Dynamic status
    wl.created_at,
...
UNION ALL
 SELECT (co.id)::text AS id,
    ...
    'purchase_order'::text AS type, -- Distinguish from simple deposits
