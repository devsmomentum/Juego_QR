-- 1. Drop the station tokens table
DROP TABLE IF EXISTS public.station_tokens CASCADE;

-- 2. Remove columns from existing tables
ALTER TABLE public.events DROP COLUMN IF EXISTS station_access_code;
ALTER TABLE public.user_clue_progress DROP COLUMN IF EXISTS assigned_puzzle_type;

-- 3. Drop RPC functions
DROP FUNCTION IF EXISTS public.generate_station_access_code(uuid);
DROP FUNCTION IF EXISTS public.validate_station_access(uuid, text);
DROP FUNCTION IF EXISTS public.generate_station_token(uuid, text, uuid);
DROP FUNCTION IF EXISTS public.consume_station_token(text);

-- 4. Revert get_clues_with_progress to a version without station logic
...
BEGIN
    ...
    -- Map user progress for this event
    ...
    LOOP
        ...
        -- Logic to determine if locked (only the next one after the last completed is unlocked)
        v_is_locked := NOT v_prev_completed;

        -- Force is_completed to false if locked (safety)
        IF v_is_locked THEN
        ...
        v_result := v_result || jsonb_build_object(
            ...
            'hint', v_clue.hint,
            'type', v_clue.type,
            'puzzle_type', v_clue.puzzle_type, -- Back to default
